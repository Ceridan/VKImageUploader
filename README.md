...
When you find somewhere in the web cool picture and want to post it to your VKontakte timeline you can easily do it with this extension. You should click right mouse button to open context menu and you can see there new menu item: "На стену!" (English translation should be like: "To the timeline!") and it will post image to your timeline immediatly. Extension have options page where you can choose where do you want to post image: your timeline or group timeline.
...
Russian:
VK API теперь требует явно передавать версию API. Этот проект работает со старой версией API 3.0, поэтому на самом деле данное исправление просто передает параметр с версией 3.0 ко всем запросам.
